...
module.exports = platform = (grunt) ->
  ...
  local = (platform, fn) ->
    # helpers.exec "cordova platform add #{platform} #{helpers.setVerbosity()} && cordova build #{platform} #{helpers.setVerbosity()}", fn
    helpers.exec "cordova platform add #{platform} #{helpers.setVerbosity()}", fn
